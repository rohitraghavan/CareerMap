...
def insert_userid(user_id):
    ...
    # associates sql connection, important because cursor allows us to execute
    # sql statements
    cur = con.cursor()
...
def retrieve_course(concentration):
    ...
    cur = con.cursor()
    results = cur.execute(
        "SELECT course_name FROM courses_per_concentration_vw WHERE concentration_name = '" + concentration + "'").fetchall()
    return results
